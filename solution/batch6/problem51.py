...
Goal: Find the smallest N-digit prime which, by replacing K digits of the number
with the same digit, is part of an L-prime value family at minimum. K digits are
not necessarily adjacent & leading zeroes should not be considered.

Constraints: 2 <= N <= 7, 1 <= K <= N, and
             1 <= L <= 8

Prime value family: A set of prime numbers generated by replacing the same digits
with a new digit.
...
def smallest_prime_digit_repl(n: int, k: int, length: int) -> list[int]:
    """
...
def smallest_8_prime_family() -> int:
    """
    Project Euler specific implementation that requires the smallest prime that
    is part of an 8-prime value family to be returned. The amount of digits to be
    replaced is not specified, so all values of K should be attempted for numbers
    greater than 56009 (the next prime after 56003, which is the smallest prime
    to be in a 7-prime value family).

    It is optimised by only replacing digits in {0, 1, 2} as anything greater
    would not be able to generate 7 more primes of greater value.
    """
