...
Euler's Quadratic formula -> `n^2 + n + 41, produces 40 primes for the
consecutive values of [0, 39].
The formula -> n^2 - 79n + 1601, produces 80 primes for the consecutive
values of [0, 79].
...
def quad_prime_coeff(max_n: int) -> (int, int, int):
    ...
    -   When n = 0, formula -> 0^2 + 0 + b = b, which means that b must
