...
Quadratic Formula: :math:`n^2 + an + b`, where |a| < N & |b| <= N.

Euler's Quadratic formula -> :math:`n^2 + n + 41`, produces 40 primes for the
consecutive values of [0, 39].
The formula -> :math:`n^2 - 79n + 1601`, produces 80 primes for the consecutive
values of [0, 79].

e.g.: N = 42
      formula -> :math:`n^2 - n + 41`, produces 42 primes
      result = -1 41
"""
...
def quad_prime_coeff(max_n: int) -> (int, int, int):
    ...
    -   When n = 0, formula -> :math:`0^2 + 0 + b = b`, which means that b must
        be a prime number itself.

    -   When n = 1, formula -> :math:`1^2 + a + b`, so, with b being prime:
