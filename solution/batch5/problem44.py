...
Goal: For a given K, find all :math:`P_n`, where n < N, such that
:math:`P_n - P_{n-K} || P_n + P_{n-K}` is also pentagonal.
...
e.g. :math:`P_7 = 70` is special in that, if K = 2:
     :math:`P_7 - P_5 = 35 = P_5`; and, if K = 3:
     :math:`P_7 + P_4 = 92 = P_8`.
...
def pentagon_numbers_PE() -> int:
    ...
    :math:`|P_x - P_y|`, for a pair of pentagonal numbers whose sum and difference
     are both pentagonal.

    Surprisingly, the first eligible pair found has the smallest difference:

    :math:`P_x` = 7_042_750, where x = 2167, and

    :math:`P_y` = 1_560_090, where y = 1020.
    """
